...
#### 1. How do I set up an AI provider?

RollOrNot requires you to configure your own AI model API key before using AI features.

1. Open the app and go to **Settings** → **AI Model Configuration**
2. Tap **Add Provider** and select your preferred AI service (OpenAI, DeepSeek, Google Gemini, Claude, Qwen, Doubao, ZhipuAI, GitHub Copilot, Ollama, LM Studio, llama.cpp)
3. Enter your API key and the base URL (default values are provided for each provider)
4. Tap **Save** and test the connection
5. Select this provider as the active service

> **Note**: The app does not provide AI model quota. You need to register with your chosen provider and obtain your own API key.

#### 2. Why is AI analysis not working?

- Check that you have configured at least one AI provider with a valid API key in **Settings** → **AI Model Configuration**
- Ensure your device has an active internet connection (except when using local models like Ollama)
- Verify that the API key you entered is correct and has sufficient quota
- Try testing the connection in **Settings** → **AI Model Configuration** → tap the provider → **Test Connection**
...
- **Offline with local models**: If you have a local AI service running (Ollama, LM Studio, or llama.cpp), all AI features work offline
- **Without any AI model**: You can still browse your question bank, view saved content, and manage chapters
- **Online features**: Cloud AI providers (OpenAI, DeepSeek, etc.) require internet access
...
使用 AI 功能前，您需要自行配置 AI 模型的 API 密钥。

1. 打开 App，前往 **设置** → **AI 模型配置**
2. 点击 **添加提供商**，选择您偏好的 AI 服务（OpenAI、DeepSeek、Google Gemini、Claude、通义千问、豆包、智谱清言、GitHub Copilot、Ollama、LM Studio、llama.cpp）
3. 输入您的 API 密钥和基础地址（每个提供商均有默认值）
4. 点击 **保存** 并测试连接
5. 将该提供商设为启用服务

> **注意**：本 App 不提供 AI 模型额度，您需自行向所选提供商注册并获取 API 密钥。

#### 2. 为什么 AI 分析无法使用？

- 请确认您已在 **设置** → **AI 模型配置** 中配置了至少一个有效的 AI 提供商
- 确保设备有网络连接（使用 Ollama 等本地模型时除外）
- 检查 API 密钥是否正确且额度充足
- 可在 **设置** → **AI 模型配置** → 点击该提供商 → **测试连接** 进行验证
...
可以，但有一定限制：
- **离线使用本地模型**：若您有本地 AI 服务（Ollama、LM Studio 或 llama.cpp），所有 AI 功能均可离线使用
- **不使用任何 AI 模型**：仍可浏览题库、查看已保存内容和管理章节
- **云端 AI 功能**：使用 OpenAI、DeepSeek 等云端服务时需要网络连接
...
使用 AI 功能前，您需要自行配置 AI 模型的 API 金鑰。

1. 打開 App，前往 **設定** → **AI 模型配置**
2. 點擊 **新增提供商**，選擇您偏好的 AI 服務（OpenAI、DeepSeek、Google Gemini、Claude、通義千問、豆包、智譜清言、GitHub Copilot、Ollama、LM Studio、llama.cpp）
3. 輸入您的 API 金鑰和基礎網址（每個提供商均有預設值）
4. 點擊 **儲存** 並測試連線
5. 將該提供商設為啟用服務

> **注意**：本 App 不提供 AI 模型額度，您需自行向所選提供商註冊並取得 API 金鑰。

#### 2. 為什麼 AI 分析無法使用？

- 請確認您已在 **設定** → **AI 模型配置** 中設定了至少一個有效的 AI 提供商
- 確保裝置有網路連線（使用 Ollama 等本地模型時除外）
- 檢查 API 金鑰是否正確且額度充足
- 可在 **設定** → **AI 模型配置** → 點擊該提供商 → **測試連線** 進行驗證
...
可以，但有部分限制：
- **離線使用本地模型**：若您有本機 AI 服務（Ollama、LM Studio 或 llama.cpp），所有 AI 功能皆可離線使用
- **不使用任何 AI 模型**：仍可瀏覽題庫、查看已儲存內容和管理章節
- **雲端 AI 功能**：使用 OpenAI、DeepSeek 等雲端服務時需要網路連線
